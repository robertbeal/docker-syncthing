...
def host(request):
    ...
    docker_id = subprocess.check_output(['docker', 'run', '--rm', '-d', 'image-under-test']).decode().strip()
...
def test_version(host):
    assert os.environ.get('VERSION','0.14.52') in host.check_output("/app/syncthing --version")
